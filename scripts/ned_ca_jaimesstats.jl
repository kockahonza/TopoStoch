...
"""
function classify_acs_jaimesstats(g, acs=attracting_components(g))
    acs_single = []
    acs_eq = []
    acs_neq = []
    acs_eqcyc = []
    ...
    for (ac_i, ac) in enumerate(acs)
        ...
        if eq_class == :single
            ...
            elseif eq_class == :neq
                ...
                throw(ErrorException("something went wrong in classify_acs_jaimesstats"))
            end
        end
    end
...
classify_acs_jaimesstats(gm::AbstractGraphModel, args...) = classify_acs_jaimesstats(graph(gm), args...)

function make_jaimesstats_df(Ns=3:6, codes=ca_ucodes_bydeg())
    df = DataFrame(;
        code=Int[],
        N=Int[],
        numstates=Int[],
    ...
    for code in codes
        for N in Ns
            ...
            ac_is_by_class = classify_acs_jaimesstats(g, acs)
