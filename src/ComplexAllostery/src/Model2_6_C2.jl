...
function makeCAGM(N, vars::SystemVars{F}=vars_base(), symmetry=Loop(), B=1; kwargs...) where {F}
    ...
    # First may as well precompute the new divisors for Model 3
    max_monomeric_energy = simplify(maximum(ca.energy_matrices.monomer))
    dB1 = exp(((vars.thetas[1, 1] + vars.thetas[1, 2]) * max_monomeric_energy) / (2 * vars.kT))
    dB2 = exp(((vars.thetas[2, 1] + vars.thetas[2, 2]) * max_monomeric_energy) / (2 * vars.kT))
    dC = exp(((vars.thetas[3, 1] + vars.thetas[3, 2]) * vars.energies[3]) / vars.kT)
    ...
    for vertex in 1:numstates(ca)
        ...
        for i in 1:ca.N
            for new_c in (state.conformations[i]+1):ca.C
                ...
                rf = vars.rs[3] * exp(exp_term_f / vars.kT) / dC
                ...
                rb = vars.rs[3] * exp(exp_term_b / vars.kT) / dC
        ...
        for i in 1:ca.N
            ...
            if cur_occ < ca.B
                ...
                begin
                    ...
                    rf = vars.rs[1][i_conf] * vars.concentrations[1] * exp(exp_term_f / vars.kT) / dB1
                    inc_edge!(ca.graph, vertex, new_vertex, rf)

                    # backward rate
                    exp_term_b = vars.thetas[1, 2] * e_i_new_state - (1 - vars.thetas[1, 1]) * (e_i_state + vars.chem_energies[1])
                    rb = vars.rs[1][i_conf] * exp(exp_term_b / vars.kT) / dB1
                    inc_edge!(ca.graph, new_vertex, vertex, rb)
                end
                ...
                begin
                    ...
                    rf = vars.rs[2][i_conf] * vars.concentrations[2] * exp(exp_term_f / vars.kT) / dB2
                    inc_edge!(ca.graph, vertex, new_vertex, rf)

                    # backward rate
                    exp_term_b = vars.thetas[2, 2] * (e_i_new_state + vars.chem_energies[3]) - (1 - vars.thetas[2, 1]) * (e_i_state + vars.chem_energies[2])
                    rb = vars.rs[2][i_conf] * vars.concentrations[3] * exp(exp_term_b / vars.kT) / dB2
                    inc_edge!(ca.graph, new_vertex, vertex, rb)
                end
            end
        end
